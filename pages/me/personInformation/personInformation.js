    ...
    if (e.detail.column == 0) { //重新渲染选项
      switch (e.detail.value) {
        case 0:
          this.setData({
            multiArray2: [
              ["头部", "前列腺", "耳鼻咽喉", "妇科", "乙肝", "血脂", "肿瘤", "内科", "血常规", "尿常规", "其他"],
              ["脑部CT", "脑电图", "头颅磁共振", "同位素扫描"]
            ]
          })
          break
        case 1:
          this.setData({
            multiArray2: [
              ["头部", "前列腺", "耳鼻咽喉", "妇科", "乙肝", "血脂", "肿瘤", "内科", "血常规", "尿常规", "其他"],
              ["前列腺增生", "前列腺炎", "前列腺癌", "前列囊肿", "前列腺脓肿", "前列结核", "前列腺结石", "前列腺B超"]
            ]
          })
          break
        case 2:
        ...
        case 5:
          this.setData({
            multiArray2: [
              ["头部", "前列腺", "耳鼻咽喉", "妇科", "乙肝", "血脂", "肿瘤", "内科", "血常规", "尿常规", "其他"],
              ["总胆固醇", "甘油三脂", "高密度脂蛋白-胆固醇", "低密度脂蛋白-胆固醇"]
            ]
          })
          break
        case 6:
        ...
        case 10:
          this.setData({
            multiArray2: [
              ["头部", "前列腺", "耳鼻咽喉", "妇科", "乙肝", "血脂", "肿瘤", "内科", "血常规", "尿常规", "其他"],
              ["抽血化验", "血管检查", "骨密度", "X线检查", "胃部", "胆囊", "肝脏", "胰腺", "脾脏", "肾脏", "内胆管", "总胆管", "肝门静脉", "胰腺", "脂肪肝", "肝硬化", "肝胆结石", "不明原因腹痛"]
            ]
          })
          break
      }
      ...
      wx.cloud.callFunction({
        ...
        if (res.result.data == null) {
          ...
          this.setData({
            ...
            department: this.data.list1[res.result.data.department[0]].name + ' ' + this.data.list1[res.result.data.department[0]].items[res.result.data.department[1]],
      ...
      wx.cloud.callFunction({
        ...
        if (res.result.data == null) {
          ...
          this.setData({
            ...
            price:res.result.data.price,
            department: this.data.list2[res.result.data.department[0]].name + ' ' + this.data.list2[res.result.data.department[0]].items[res.result.data.department[1]],
            checkbox:temp,
            experience:res.result.data.personalExperience
          })
        }
      ...
      success(res) {
        if (res.confirm) {
          ...
          if (flagnull == true) {
            ...
            that.setData({
              flagDisabled: true,
              department: that.data.list1[data.detail.value.department[0]].name + ' ' + that.data.list1[data.detail.value.department[0]].items[data.detail.value.department[1]]
      ...
      success(res) {
        if (res.confirm) {
          ...
          if (flagnull == true) {
            ...
            that.setData({
              flagDisabled: true,
              department: that.data.list2[data.detail.value.department[0]].name + ' ' + that.data.list2[data.detail.value.department[0]].items[data.detail.value.department[1]]
            })
          }
